...
class RealToobitClient(ToobitClient):
    ...
    def fetch_active_markets(self, market_type: MarketType) -> list[str]:
        ...
        for c in data.get("contracts", []):
            ...
            # If two contracts normalize to the same symbol (TOOBIT can
            # list more than one), keep the first and skip the rest —
            # otherwise the same normalized symbol gets scanned twice.
            if normalized not in self._perp_symbol_map:
                self._perp_symbol_map[normalized] = c["symbol"]  # e.g. "BTC-SWAP-USDT"
        return list(self._perp_symbol_map.keys())
    ...
    def fetch_candles(self, symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
        ...
        # End the window at the last fully-closed interval boundary, so
        # the currently-forming candle is never returned in the first
        # place (previously we fetched it and dropped it, which cost us
        # exactly one candle and made every symbol fall 1 short of
        # MIN_CANDLES).
        ...
        rows = [{
            ...
            # NOTE: k[6] (documented as "close time") proved unreliable in
            # practice against live TOOBIT data (came back as 0 / epoch for
            # even BTCUSDT). Deriving close_time from open_time + interval
            # is always correct regardless of what TOOBIT actually puts at
            # index 6, so we no longer read k[6] at all.
