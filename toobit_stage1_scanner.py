...
def run_scan(client: ToobitClient, market_types: tuple[MarketType, ...] = ("SPOT", "PERPETUAL")) -> dict:
    ...
    # Direct ground-truth probe: BTCUSDT should always be liquid and
    # fresh. If IT shows huge staleness numbers, the bug is in our
    # parsing, not in exchange data quality for illiquid pairs.
    try:
        probe_candles = client.fetch_candles("BTCUSDT", "5m", 5)
        probe_ob = client.fetch_orderbook("BTCUSDT")
        probe_candle_age = (datetime.now(timezone.utc) - probe_candles["close_time"].iloc[-1]).total_seconds()
        probe_full = client.fetch_candles("BTCUSDT", "5m", MIN_CANDLES)
        output["debug_btcusdt_probe"] = {
            "last_candle_close_time": str(probe_candles["close_time"].iloc[-1]),
            "candle_age_seconds": round(probe_candle_age, 1),
            "orderbook_age_seconds": round(probe_ob.age_seconds, 1),
            "orderbook_levels": probe_ob.levels_available,
            "candles_requested": MIN_CANDLES,
            "candles_received": len(probe_full),
        }
    except Exception as e:
        output["debug_btcusdt_probe"] = {"error": f"{type(e).__name__}: {e}"}
