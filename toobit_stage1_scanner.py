...
def score_symbol(data: SymbolData, btc_regime: str) -> Optional[SymbolResult]:
    ind = compute_indicators(data.candles_5m)
    if not ind:
        return None  # required indicator missing -> remove
    ...
    if momentum is None:
        return None  # RSI out of [15,85] -> remove
    ...
    if volat_result is None:
        return None  # ATR% too low -> remove
    ...
    if data.market_type == "PERPETUAL":
        if data.funding_rate_pct is None or data.open_interest_series is None:
            return None  # required futures data missing -> remove
...
def _build_symbol_data(client: ToobitClient, symbol: str, market_type: MarketType) -> Optional[SymbolData]:
    candles = client.fetch_candles(symbol, "5m", MIN_CANDLES)
    if candle_validity_check(candles, MIN_CANDLES) is not None:
        return None

    ob = client.fetch_orderbook(symbol)
    candle_age = (datetime.now(timezone.utc) - candles["close_time"].iloc[-1]).total_seconds()
    if data_freshness_check(ob.age_seconds, ob.age_seconds, candle_age) is not None:
        return None
...
def run_scan(client: ToobitClient) -> dict:
    """The single function to call for a full Stage-1 scan. Wire your
    real ToobitClient in and call run_scan(client)."""
    ...
    for market_type in ("SPOT", "PERPETUAL"):
        ...
        for symbol in symbols:
            ...
            data = _build_symbol_data(client, symbol, market_type)
            if data is None:
                removed_counts[market_type] += 1
                continue

            result = score_symbol(data, btc_regime)
            if result is None:
                removed_counts[market_type] += 1
                continue
